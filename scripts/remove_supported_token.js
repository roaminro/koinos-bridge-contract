...
const TOKEN_ADDR = '1KazZFUnZSLjeXq2QrifdnYqiBvA7RVF3G';
...
async function main() {
  ...
  let buffer = addRemoveActionHashProto.encode({
    action: actionIdProto.values['remove_supported_token'],
  ...
  const result = await bridgeContract.functions.remove_supported_token({
    signatures,
    token: TOKEN_ADDR,
    expiration
  });
  ...
  console.log('removed support for token ', TOKEN_ADDR);
}
