...
async function main() {
  ...
  let buffer = addRemoveActionHashProto.encode({
    ...
    address: utils.decodeBase58(WRAPPED_TOKEN_ADDR),
    nonce,
  ...
  const result = await bridgeContract.functions.add_supported_wrapped_token({
    signatures,
    token: WRAPPED_TOKEN_ADDR,
    expiration
  });

  await result.transaction.wait();

  console.log('added support for wrapped token ', WRAPPED_TOKEN_ADDR);
}
